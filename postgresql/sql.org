...
***** Example
    ...
    | public            | sampl_serial  | table | postgres |
    | public            | sample        | table | postgres |
    | public            | sample_big    | table | postgres |
    | public            | sample_serial | table | postgres |
...
** Basic operators and functions
  ...
  - =jsonb_build_object=
   #+BEGIN_SRC sql
     select jsonb_build_object('name', 'Bob');
   #+END_SRC
...
** Window Functions
  ...
  [[https://www.postgresql.org/docs/9.1/tutorial-window.html][Window Functions]] - performs a calculation across a set of table rows that are somehow related to the current row.

  #+BEGIN_SRC sql
    select EXTRACT(YEAR from d), v
          , lag(v, -1) over (order by d desc) prev
          , v - lag(v, -1) over (order by d desc)  inc
    from (values ('2021-01-01'::timestamp, 10)
               , ('2020-01-01'::timestamp, 8)
               , ('2019-01-01'::timestamp, 10)
               , ('2018-01-01'::timestamp, 5))  t(d, v)
  #+END_SRC
